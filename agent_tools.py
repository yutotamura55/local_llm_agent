...
def write_file(path: str, content: str) -> str:
    """指定したファイルに内容を書き込む(新規作成 or 上書き)。呼び出し前提: confirm済み"""
    try:
...
WRITE_SUBCOMMANDS = {
    "git": {"commit", "push", "checkout", "reset", "merge", "rebase",
            "add", "rm", "clean", "stash", "tag", "branch"},
...
def preview_edit(path: str, search: str, replace: str) -> EditPreview:
    """実際には書き込まず、一意性チェックとdiff生成だけ行う"""
    try:
        with open(path, "r", encoding="utf-8") as f:
            original = f.read()
    except FileNotFoundError:
    ...
    if count == 0:
        return EditPreview(ok=False, message="search文字列がファイル内に見つかりませんでした。完全に一致する文字列を指定してください。")
    if count > 1:
        return EditPreview(ok=False, message=f"search文字列がファイル内に{count}箇所存在し一意に特定できません。前後の文脈を含めて一意になるようsearchを広げてください。")
...
def edit_file(path: str, search: str, replace: str) -> str:
    """実際に置換を実行する。呼び出し前提: preview_editでok=True確認済み・confirm済み"""
    try:
        with open(path, "r", encoding="utf-8") as f:
            original = f.read()
    except Exception as e:
...
TOOLS = [
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {"directory": {"type": "string", "description": "一覧を取得したいディレクトリのパス"}},
                "required": ["directory"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {"path": {"type": "string", "description": "読み込みたいファイルのパス"}},
                "required": ["path"],
            },
        },
    },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "search": {"type": "string", "description": "置換対象の完全一致文字列(ファイル内で一意である必要がある)"},
